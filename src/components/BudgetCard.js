...
export default function BudgetCard({ name, amount, max,gray,hideButtons,onAddExpenseClick,onViewExpenseClick }) {
    ...
    return (
        <Card className={classNames.join(" ")}>
            <Card.Body>
                <Card.Title className="d-flex justify-content-between align-tiems-baseline fw-normal mb-3">
                    <div className="me-2">{name}</div>
                    <div className="d-flex align-items-baseline">
                      {max &&  (<span className="text-muted fs-6 ms-1">{currencyFormatter.format(amount)}/{currencyFormatter.format(max)}
                    </span>)}
                    </div>
                </Card.Title>
            ...
            {!hideButtons &&( <Stack direction= "horizontal" gap="2" className="mt-4"   >
                <Button variant="outline-primary" class="ms-auto " onClick={onAddExpenseClick}>Add Expense</Button>
